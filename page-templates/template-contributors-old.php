...
/**
 * Template Name: Contributors Old
 *
...
<div class="pol-contributors-page">
    ...
    if (isset($_GET['nom_de_plume'])) {

        if (!empty($_GET['nom_de_plume']) && $_GET['nom_de_plume'] != '') { ?>
            <script>
                if (window.location.href.includes('/page/')) {
                    window.location.href = "/contributors/?nom_de_plume=<?php echo $_GET['nom_de_plume']; ?>";
                }
            </script>
        <?php }
    }
        ...
        if (!is_array($meta_key)) {
            ...
            foreach ($query_arr as $q) {
                ...
                $new_sql .=
                    ...
                    WHERE um.meta_key LIKE '" . $meta_key . "' AND um.meta_value LIKE '%" . $q . "%' 
                ";
            }
        } else {

            $new_sql = '';
            if($meta_key == "grew_up_languages"){
                foreach ($query_arr as $q) {
                    $new_sql .= ($search_sql != '') ? ' UNION ALL ' : '';
                    $new_sql .=
                    "
    ...
    if (
        ...
        if (!empty($search_term)) {
            ...
            //     UNION  
            //     SELECT DISTINCT(um.user_id) as ID FROM bny_usermeta um 
            //     WHERE (um.meta_key LIKE 'nom_de_plume' AND um.meta_value LIKE '%" . $search_term . "%'
            //     OR um.meta_key LIKE 'nickname' AND um.meta_value LIKE '%" . $search_term . "%')
            // ";
            ...
            $search_sql .=
                ...
                SELECT DISTINCT(p.post_author) as ID FROM bny_posts p WHERE p.post_title LIKE '%" . $search_term . "%' 
                UNION  
                SELECT ID FROM bny_users
                WHERE display_name LIKE '%" . $search_term . "%'
            ";
            $filter_count++;
        }
        ...
        //         FROM bny_postmeta pm
        //         INNER JOIN bny_posts p ON pm.post_id = p.ID
        //         WHERE pm.meta_key LIKE 'fav_authors' AND pm.meta_value LIKE '%" . $search_fav_author . "%' 
        //     ";
        //     $filter_count++;
        // }
        ...
        //         FROM bny_postmeta pm
        //         INNER JOIN bny_posts p ON pm.post_id = p.ID
        //         WHERE pm.meta_key LIKE 'fav_subject_to_write_about' AND pm.meta_value LIKE '%" . $search_subject . "%' 
        //     ";
        //     $filter_count++;
        // }
        ...
        //         FROM bny_usermeta um
        //         WHERE 
        //         (um.meta_key LIKE 'current_location' AND um.meta_value LIKE '%" . $search_curr_loc . "%' )
        //         OR
        //         (um.meta_key LIKE 'current_location_city' AND um.meta_value LIKE '%" . $search_curr_loc . "%' )
        //         OR
        //         (um.meta_key LIKE 'current_location_nation' AND um.meta_value LIKE '%" . $search_curr_loc . "%' )
        //     ";
        //     $filter_count++;
        // }
    

        $search_sql .= "LIMIT " . $number . " OFFSET " . $offset . ";";
        ...
        if(is_user_logged_in() && get_current_user_id() == 14){
            // var_dump($search_sql);
    ...
    <form method="GET" action="" id="filter-contributors">
        <input type="text" name="nom_de_plume" id="search-by-author-title"
            placeholder="Search by author or title of story" value="<?php echo $search_term; ?>">
        <p>Advanced Search</p>
        ...
        <div class="contributors-form-options">
            ...
            <!-- <input type="text" name="curr_loc" placeholder="netherlands" value="<?php //echo $search_curr_loc; ?>"> -->
            <!-- <select name="curr_loc[]" id="curr_loc" multiple="multiple">
                <?php
                // foreach ($search_curr_loc as $lang) {
                //     echo '<option value="' . $lang . '" >' . $lang . '</option>';
                // } 
                ?>
            </select> -->
        ...
        <!-- <div class="contributors-form-options">
            ...
            <select name="lang">
                ...
                <?php
                // foreach ($place_languages as $lang) {
                //     $is_selected = ($lang == $search_story_lang) ? 'selected' : '';
                //     echo '<option value="' . $lang . '" ' . $is_selected . ' >' . $lang . '</option>';
                // }
                ?>
            </select>
            <br>
        ...
        <div class="contributors-form-options">
            ...
            <select name="loc_id">
                ...
                //     if (get_post_meta($location->ID, 'where_does', true) == 'geo_loc') {
                //         $is_selected = ($location->ID == $search_story_loc_id) ? 'selected' : '';
                //         echo '<option value="' . $location->ID . '" ' . $is_selected . ' >' . $location->post_title . '</option>';
                //     }
                // }
                ?>
            </select>
    ...
    function keep_repeated_elements($array)
    {
        $counted_array = array_count_values($array);
        $filtered_array = array_filter($counted_array, function ($value) {
            return $value > 1;
        });
    ...
    if ($filter_count > 1) {
        $user_id_list = keep_repeated_elements($user_id_list);
    }
   ...
   // $rae_check =  get_user_meta(get_current_user_id(), 'rae_approved', true) == 1 ;
    ...
    foreach ($user_id_list as $q) {
        ...
        // $ind_username = $curr_user->user_login;
        $ind_username = $curr_user->display_name;
        // $ind_full_name = get_user_meta($q, 'first_name', true) . ' ' . get_user_meta($q, 'last_name', true);
        // $display_name = (empty($ind_nom_de_plume)) ? $ind_username : $ind_nom_de_plume;
    ...
    if ($total_users > $total_query) {
        ...
        echo paginate_links(
            array(
                'base' => get_bloginfo('url') . '/contributors/page/%#%/',
                'format' => get_bloginfo('url') . '/contributors/page/%#%/',
                // 'format' => 'page/%#%/',
